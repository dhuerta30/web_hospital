...
$(document).on("artify_after_ajax_action", function(event, obj, data){
	...
	if(action == "add"){
		...
		$.ajax({
            type: "POST",
            url: '{{ $_ENV["BASE_URL"] }}editar_iconos_submenu',
            dataType: "json",
            data: { id: id },
			beforeSend: function() {
				$("#artify-ajax-loader").show();
...
function sortable(){
	$(".artify-table tbody").sortable({
	  ...
	  update: function(event, ui){
		...
		updateUrl = '{{ $_ENV["BASE_URL"] }}actualizar_orden_menu';
...
function sortableSubmenu(){
	$(".submenutable tbody").sortable({
	  ...
	  update: function(event, ui){
		...
		updateUrl = '{{ $_ENV["BASE_URL"] }}actualizar_orden_submenu';
...
$('#Submenu-tab').on('shown.bs.tab', function (e) {
	$.ajax({
		type: "POST",
		url: '{{ $_ENV["BASE_URL"] }}cargar_vista_submenu',
		dataType: "HTML",
		beforeSend: function() {
