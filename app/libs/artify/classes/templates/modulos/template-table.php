            ...
            ?>
                <tr data-id="<?php if (isset($rows[$pk])) echo $rows[$pk]; ?>" id="artify-row-<?php echo $rowcount; ?>" class="artify-data-row <?php if (isset($rows[0]["class"])) echo $rows[0]["class"]; ?>" <?php if (isset($rows[0]["style"])) echo $rows[0]["style"]; ?>>
                    ...
                    ?>
                        ...
                        $url = preg_replace_callback('/{([^}]+)}/u', function ($matches) use ($rows) {
                            $field = $matches[1]; // El campo dentro de las llaves
                        
                            // Verificar si el campo existe y limpiarlo
                            if (isset($rows[$field])) {
                                return limpiarTexto($rows[$field]); // Devuelve el valor del campo limpiado
                            }
                            
                            return $matches[0]; // Devuelve el marcador original si no existe
                        }, $url);
