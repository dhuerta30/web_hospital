...
<style type="text/css">
    ...
    .xinvoice-box table tr.xitem td{
        border-bottom: 1px solid #88b267;
    }
    ...
    span.xinvoice-invoice-label{
        color:#88b267;
    }
    span.xinvoice-document-data{
        color:#88b267;
        font-size: 24px
    }
        ...
        <table cellpadding="0" cellspacing="0" class="xinvoice-header" style="<?php if (isset($data["header"]["style"])) echo $data["header"]["style"]; ?>">
            <tr>
                ...
                <td class="xinvoice-invoice-details">
                     <?php
                    if (!isset($data["document"]["title"]["display"]) || (isset($data["document"]["title"]["display"]) && $data["document"]["title"]["display"])) {
                        if (isset($data["document"]["title"]["data"]))
                            echo "<span class='xinvoice-document-data'>".$data["document"]["title"]["data"]. "</span><br/>";
                    }
                ...
                if (!isset($data["from"]["display"]) || (isset($data["from"]["display"]) && $data["from"]["display"] ))
                    foreach ($data["from"] as $key => $val) {
                        ...
                        if (isset($val["display"]) && $val["display"] === false)
                            continue;
                        if (isset($val["style"])) $style = $val["style"];
                        if (isset($val["label"])) echo "<span class='xinvoice-invoice-label'>".$val["label"]. "</span><hr style=''/>";
                        if (isset($val["data"]))  echo "<span class='xinvoice-invoice-data' style='". $style ."'>".$val["data"]. "</span><br/>";
                    }
                ?>
            </td>
